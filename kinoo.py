...
# -*- coding: utf-8 -*-
"""
KINO BOT — to'liq qayta yozilgan versiya
- aiogram 3.22 mos
- Termux friendly
- O'zbekcha izohlar va emoji bilan
- PART1 / PART2 / PART3 tarzida bo'lingan (ketma-ket nusxa qiling)
"""
...
BOT_TOKEN = os.getenv("BOT_TOKEN", "8523015883:AAH6Ymnh5P-31hQl0QzbAgpDO0icOj3P708")
if not BOT_TOKEN:
    raise SystemExit("BOT_TOKEN kerak. export BOT_TOKEN=\"<token>\"")
...
DB_FILE = os.getenv("DB_FILE", "kini98o_full.db")
VALIDATION_TTL = int(os.getenv("VALIDATION_TTL", "3600"))  # sekund
REQUIRED_CHANNEL = os.getenv("REQUIRED_CHANNEL", "")  # @kanal yoki bo'sh

# Bot: parse_mode not set globally; send with parse_mode=None for safety
bot = Bot(token=BOT_TOKEN, default=DefaultBotProperties(parse_mode=None))
dp = Dispatcher()

# Admin simple FSM state holder
admin_states: Dict[int, Dict[str, Any]] = {}

# -------------------- UTIL HELPERS --------------------
def make_tg_url(val: Optional[str]) -> Optional[str]:
    """@username yoki invite'ni to'liq https://t.me/... ga o'tkazadi."""
    if not val:
        return None
    v = val.strip()
...
# -------------------- DB INIT + MIGRATION --------------------
async def init_db():
    """DB yaratish va basic migratsiyalar."""
    async with aiosqlite.connect(DB_FILE) as db:
        ...
        try:
            ...
            logger.exception("Migration attempt failed (continuing)")

# -------------------- part2 starts here --------------------
# DB helpers: settings, users, groups, movies, series, episodes
...
async def add_movie_db(code: str, title: str, file_id: str, file_type: str,
    ...
    async with aiosqlite.connect(DB_FILE) as db:
        ...
        await db.execute("""
            INSERT OR REPLACE INTO movies(code, title, file_id, file_type, year, genre, language, description, country, downloads)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, (code, title, file_id, file_type, year, genre, language, description, country, downloads))
        await db.commit()
...
async def get_movie_db(code: str):
    async with aiosqlite.connect(DB_FILE) as db:
        cur = await db.execute("SELECT title, file_id, file_type, year, genre, language, description, country, COALESCE(downloads,0) FROM movies WHERE code = ?", (code,))
        r = await cur.fetchone()
    return r
...
def admin_main_kb() -> ReplyKeyboardMarkup:
    """Admin asosiy menyu (O'zbekcha tugmalar)."""
...
def collect_episodes_kb() -> ReplyKeyboardMarkup:
    return ReplyKeyboardMarkup(
        keyboard=[
            [KeyboardButton(text="Tugatish ✅"), KeyboardButton(text="Cancel")]
        ], resize_keyboard=True
    )

async def build_movie_kb(code: str, title: str) -> InlineKeyboardMarkup:
    """
    Async yordamchi: settings_get('codes_link') dan share link oladi va
    InlineKeyboardMarkup qaytaradi. Shu bilan share tugmasi ko'rinadi.
    """
    codes_link = await settings_get("codes_link")
    rows = []
    # Agar admin sozlagan codes_link bo'lsa, ulashish knopkasi unga yo'naltirsin
    if codes_link:
        # sharable text: kinoning kodi va nomi
        share_text = f"Kod: {code} — {title}"
        share_url = "https://t.me/share/url?url=&text=" + urllib.parse.quote_plus(share_text) + "&to=" + urllib.parse.quote_plus(codes_link)
        ...
        rows.append([InlineKeyboardButton(text="🔁 Ulashish", url=share_url)])
    # Yashirish tugmasi
    rows[0].append(InlineKeyboardButton(text="❌ Yashirish", callback_data=f"movie:hide:{code}"))
    return InlineKeyboardMarkup(inline_keyboard=rows)

async def safe_send(user_id: int, text: str, reply_markup=None) -> bool:
    """Xabar yuborish — parse_mode=None bilan va try/except."""
    try:
...
async def cb_movie_hide(cq: CallbackQuery):
    try:
        # Xabarni o'chirishga harakat qilamiz (foydalanuvchi uchun)
...
# groups check helper (used in several places)
...
async def admin_text_handler(message: Message):
    ...
    # handle stateful steps
    if st:
        action = st.get("action")
        step = st.get("step")

        # GROUP flows (same as part2 logic)
        ...
        if action == "add_movie" and step == "wait_genre_kino":
            ...
            st = admin_states.get(ADMIN_ID, {})
            st.update({"step": "wait_country_kino", "genre": genre})
            admin_states[ADMIN_ID] = st
        ...
        if action == "add_movie" and step == "wait_description_kino":
            ...
            if not st:
                ...
                return
            # kode yaratish va DB ga yozish
            file_id = st.get("file_id"); ftype = st.get("file_type")
            title = st.get("title"); language = st.get("language"); genre = st.get("genre"); country = st.get("country"); year = st.get("year")
            ...
            await add_movie_db(code, title, file_id, ftype, year, genre, language, description, country)
            # xabar: adminga va agar kerak bo'lsa kanalga ulashish
            await safe_send(ADMIN_ID, f"✅ Kino saqlandi! Kod: {code}", reply_markup=admin_main_kb())
            return

        # ---------- Serial: media-first collection ----------
        if action == "add_movie" and step == "wait_series_meta":
            meta = (message.text or "").strip()
            if not meta:
                await safe_send(ADMIN_ID, "Iltimos serial nomi va tilini yuboring (misol: Nom — o'zbekcha).", reply_markup=admin_flow_kb())
                return
            ...
            admin_states[ADMIN_ID] = {"action": "add_movie", "step": "collect_episodes", "series_code": series_code, "series_title": title, "series_language": language, "episodes": []}
            await safe_send(ADMIN_ID, f"✅ Serial yaratildi: {title} (kod: {series_code}).\n📤 Endi EPIZOD fayllarini yuboring. Hammasini yuborgach \"Tugatish ✅\" ni bosing.", reply_markup=collect_episodes_kb())
            return

        if action == "add_movie" and step == "collect_episodes":
            # agar Tugatish bosilgan bo'lsa
            if text == "Tugatish ✅":
                st = admin_states.pop(ADMIN_ID, None)
                if not st:
                    await safe_send(ADMIN_ID, "Xatolik: holat topilmadi.", reply_markup=admin_main_kb())
                ...
                eps = st.get("episodes", [])
                if not eps:
                    await safe_send(ADMIN_ID, "❗ Hech qanday epizod yuborilmadi.", reply_markup=admin_main_kb())
                    return
                for idx, (fid, ftype) in enumerate(eps, start=1):
                    await add_episode_db(st["series_code"], idx, fid, ftype, None)
                await safe_send(ADMIN_ID, f"🎉 Serial saqlandi: {st['series_title']} (kod: {st['series_code']}). Jami {len(eps)} qism qo'shildi.", reply_markup=admin_main_kb())
                return
            # aks holda media qabul qilamiz (ko'p marotaba)
            file_id = None; ftype = None
            if message.video:
                file_id = message.video.file_id; ftype = "video"
            elif message.document:
                ...
                await safe_send(ADMIN_ID, "Iltimos epizod faylini yuboring yoki 'Tugatish ✅' ni bosing.", reply_markup=collect_episodes_kb())
                return
            st = admin_states.get(ADMIN_ID)
            if not st:
                await safe_send(ADMIN_ID, "Xatolik: holat topilmadi.", reply_markup=admin_main_kb())
            ...
            admin_states[ADMIN_ID] = st
            epnum = len(eps)
            await safe_send(ADMIN_ID, f"✅ {epnum}-qism saqlandi. Kod: {st['series_code']}-{epnum}\nYana fayl yuboring yoki 'Tugatish ✅'ni bosing.", reply_markup=collect_episodes_kb())
            return
    ...
    # No active state: admin main commands (labels are in Uzbek)
    ...
    if text == "Serial qo'shish 📺":
        # media-first serial qo'shish (admin: avval fayllarni yuboradi, Tugatish bosadi, keyin nom/til so'raladi)
        admin_states[ADMIN_ID] = {"action": "add_movie", "step": "collect_episodes_pending_meta"}
        await safe_send(ADMIN_ID, "📤 Serial qo'shish: Endi EPIZOD fayllarini yuboring (hammasini). Hammasini yuborgach 'Tugatish ✅' ni bosing. (Keyin nom/til so'raladi)", reply_markup=collect_episodes_kb())
        return
...
# Special: collect_episodes_pending_meta path (serial: admin uploads files first, then presses 'Tugatish' which triggers asking meta)
@dp.message(lambda m: m.from_user is not None and m.from_user.id == ADMIN_ID and admin_states.get(ADMIN_ID, {}).get("step") == "collect_episodes_pending_meta")
async def admin_collect_episodes_pending_meta(message: Message):
    st = admin_states.get(ADMIN_ID)
    if not st:
        await safe_send(ADMIN_ID, "Xatolik: holat yo'q.", reply_markup=admin_main_kb()); return
    text = (message.text or "").strip()
    # if admin presses 'Tugatish ✅' -> ask for meta (title & language)
    if text == "Tugatish ✅":
        # move to meta-ask state
        admin_states[ADMIN_ID] = {"action": "add_movie", "step": "wait_series_meta_pending", "temp_eps": st.get("episodes", [])}
        await safe_send(ADMIN_ID, "✅ Hammasi saqlandi. Endi SERIAL NOMI va TILINI yuboring (misol: Nom — o'zbekcha):", reply_markup=admin_flow_kb())
        return
    # otherwise accept media
    file_id = None; ftype = None
    if message.video:
        file_id = message.video.file_id; ftype = "video"
    elif message.document:
    ...
    # append to temp episodes list
    eps = st.get("episodes", [])
    eps.append((file_id, ftype))
    st["episodes"] = eps
    admin_states[ADMIN_ID] = st
    await safe_send(ADMIN_ID, f"✅ {len(eps)}-qism saqlandi. Yuborishni davom ettiring yoki 'Tugatish ✅'ni bosing.", reply_markup=collect_episodes_kb())
    return

# handle meta after pending collection
@dp.message(lambda m: m.from_user is not None and m.from_user.id == ADMIN_ID and admin_states.get(ADMIN_ID, {}).get("step") == "wait_series_meta_pending")
async def admin_finish_series_pending_meta(message: Message):
    meta = (message.text or "").strip()
    if not meta:
...
async def cmd_start(message: Message):
    """Start: foydalanuvchini bazaga qo'shish va darhol subscripiton tekshirish."""
    ...
    if ok:
        ...
        await safe_send(message.from_user.id, "👋 Assalomu alaykum! Tekshiruv muvaffaqiyatli. Kino kodini yuboring.")
    else:
        await invalidate_user_subscription(message.from_user.id)
        # agar REQUIRED_CHANNEL bo'lsa alohida xabar beramiz, boshqa hollarda guruhlar ro'yxati yuboriladi
        if REQUIRED_CHANNEL:
            await safe_send(message.from_user.id, f"👋 Assalomu alaykum! Iltimos quyidagi kanal/guruhlarga qo'shiling: {REQUIRED_CHANNEL}")
        else:
            kb = await groups_inline_kb(missing)
            await safe_send(message.from_user.id, "👋 Assalomu alaykum! Kodni olish uchun avvalo quyidagilarga a'zo bo'ling:", reply_markup=kb)
    # agar admin bo'lsa, admin panel yuborilsin
...
async def cmd_help(message: Message):
    txt = ("ℹ️ Yordam:\n"
           ...
           "/help — yordam\n"
           "Kodni yuboring: 123 yoki 123-1 (serial epizod)\n")
...
async def user_message_handler(message: Message):
    """Foydalanuvchidan kelgan text — kodlarni qayta ishlash (movie yoki episode)."""
    await add_user_db(message.from_user.id)
    ...
    # match movie or series listing: 123
    ...
    if m2:
        ...
        if mv:
            ...
            if need_validation:
                ...
                if not ok:
                    ...
                    return
                await update_user_last_validated(message.from_user.id, datetime.datetime.now(datetime.timezone.utc))
            # send movie: include title + blank line + description
            title, file_id, file_type, year, genre, language, desc, country, downloads = mv
            caption_parts = []
            caption_parts.append(html.escape(title or "Film"))
            # bitta bo'sh qator va description
            if desc:
                caption_parts.append("")  # blank line
                caption_parts.append(html.escape(desc))
            # metadata qism
            caption_parts.append("")  # another blank
            caption_parts.append(f"Yili: {year or '-'} | Janr: {genre or '-'} | Davlat: {country or '-'} | Til: {language or '-'}")
            caption_parts.append("") 
            caption_parts.append(f"Kod: {code}")
            caption = "\n".join(caption_parts)
            kb = await build_movie_kb(code, title or "Film")
            try:
                if file_type == "video":
                    sent = await bot.send_video(message.from_user.id, file_id, caption=caption, reply_markup=kb)
                else:
                    sent = await bot.send_document(message.from_user.id, file_id, caption=caption, reply_markup=kb)
            except Exception:
            ...
            return
        # not a movie -> maybe series listing
        meta = await get_series_meta(code)
        if not meta:
        ...
        lines = [f"{title}"]
        for num, _, _, ep_title, _ in eps:
            lang = language or "-"
            if ep_title:
                lines.append(f"{num}-qism — {lang} — {ep_title} — Kod: {code}-{num}")
            else:
                lines.append(f"{num}-qism — {lang} — Kod: {code}-{num}")
        await safe_send(message.from_user.id, "\n".join(lines))
        return

    # default fallback
    await safe_send(message.from_user.id, "ℹ️ Iltimos kino yoki serial kodini yuboring (misol: 123 yoki 123-1).")
